...
namespace COTUI
{
    public partial class MQTTForm : Form
    {
        ...
        public MQTTForm()
        {
            ...
            logger.Log(LogLevel.Info, "MQTT管理页面创建");
        }
        ...
        private void InitializeCommunicationServices()
        {
            try
            {
                ...
                logger.Log(LogLevel.Info, "通讯服务初始化完成");
            }
            catch (Exception ex)
            {
                logger.ErrorException(ex, "初始化通讯服务失败");
            }
        }
        ...
        private void MQTTForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                try
                {
                    ...
                    // 显示配置文件信息（调试用）
                    logger.Log(LogLevel.Info, "=== 配置文件信息 ===");
                    logger.Log(LogLevel.Info, config.GetConfigInfo());
                    logger.Log(LogLevel.Info, "====================");
                }
                catch (FileNotFoundException ex)
                {
                    logger.ErrorException(ex, "配置文件不存在");
                ...
                logger.Log(LogLevel.Info, "MQTT管理页面加载完成");
            }
            catch (Exception ex)
            {
                logger.ErrorException(ex, "MQTT管理页面加载失败");
        ...
        private bool ValidateConfiguration()
        {
            ...
            {
                logger.Log(LogLevel.Warn, "❌ MQTT服务器地址为空");
                isValid = false;
            }
            
            if (mqttPort < MIN_PORT || mqttPort > MAX_PORT)
            {
                logger.Log(LogLevel.Warn, $"❌ MQTT端口号无效: {mqttPort}（有效范围：{MIN_PORT}-{MAX_PORT}）");
                isValid = false;
            }
            
            if (string.IsNullOrWhiteSpace(mqttClientId))
            {
                logger.Log(LogLevel.Warn, "❌ MQTT客户端ID为空");
                isValid = false;
            }
            
            if (isValid)
            {
                logger.Log(LogLevel.Info, $"✅ 配置验证通过: {mqttServer}:{mqttPort}");
            }
            else
            {
                logger.Log(LogLevel.Warn, "⚠️ 配置验证失败，请检查Config.ini文件");
            }
        ...
        private void LoadConfiguration()
        {
            try
            {
                ...
                {
                    // ✅ 从配置文件读取MQTT配置
                    mqttServer = config.GetValue("MQTT", "Server");
                    mqttPort = config.GetIntValue("MQTT", "Port", DEFAULT_MQTT_PORT);
                    mqttClientId = config.GetValue("MQTT", "ClientID");
                    mqttUsername = config.GetValue("MQTT", "Username");
                    mqttPassword = config.GetValue("MQTT", "Password");
                    mqttTopicPrefix = config.GetValue("MQTT", "TopicPrefix");
                    
                    // ✅ 从配置文件读取TCP Socket配置
                    tcpServerHost = config.GetValue("TcpSocket", "ServerHost", "192.168.1.100");
                    tcpServerPort = config.GetIntValue("TcpSocket", "ServerPort", DEFAULT_TCP_PORT);
                    tcpEnableHeartbeat = config.GetBoolValue("TcpSocket", "EnableHeartbeat", true);
                    tcpHeartbeatInterval = config.GetIntValue("TcpSocket", "HeartbeatInterval", 30000);
                    
                    logger.Log(LogLevel.Info, $"✅ 配置已从文件加载");
                    logger.Log(LogLevel.Debug, $"   MQTT: {mqttServer}:{mqttPort}");
                    logger.Log(LogLevel.Debug, $"   TCP: {tcpServerHost}:{tcpServerPort}");
                }
                else
                {
                    ...
                    logger.Log(LogLevel.Warn, "⚠️ 配置文件不可用，使用默认配置");
                }
            }
            catch (Exception ex)
            {
                logger.ErrorException(ex, "加载配置失败");
        ...
        private void DisplayConfiguration()
        {
            try
            {
                ...
                logger.Log(LogLevel.Debug, "配置信息已显示到界面");
            }
            catch (Exception ex)
            {
                logger.ErrorException(ex, "显示配置信息失败");
            }
        }
        ...
        private void SaveConfiguration()
        {
            try
            {
                ...
                if (config != null)
                {
                    config.SetValue("MQTT", "Server", mqttServer);
                    config.SetValue("MQTT", "Port", mqttPort);
                    config.SetValue("MQTT", "Username", mqttUsername);
                    config.SetValue("MQTT", "Password", mqttPassword);
                    
                    // 写入文件
                    config.SaveConfig();
                    
                    logger.Log(LogLevel.Info, $"✅ MQTT配置已保存: {mqttServer}:{mqttPort}");
                }
                else
                {
                    logger.Log(LogLevel.Warn, $"⚠️ 配置已更新（仅内存，未保存到文件）");
                }
            }
            catch (FormatException ex)
            {
                logger.ErrorException(ex, "保存MQTT配置失败：格式错误");
                MessageBox.Show(
                    $"配置格式错误！\n\n" +
                    $"端口输入: {txtPort.Text}\n" +
                    $"错误详情: {ex.Message}\n\n" +
            ...
            {
                logger.ErrorException(ex, "保存MQTT配置失败");
        ...
        private async void btnConnect_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                logger.Log(LogLevel.Info, $"开始连接MQTT: {mqttServer}:{mqttPort} (ClientID: {mqttClientId})");
            ...
            {
                logger.ErrorException(ex, "MQTT连接失败：参数为空");
                MessageBox.Show(
                    $"连接失败：参数为空\n\n" +
                    $"错误详情: {ex.Message}\n\n" +
                    $"当前配置:\n" +
            ...
            {
                logger.ErrorException(ex, "MQTT连接失败");
                MessageBox.Show(
                    $"连接失败：{ex.Message}\n\n" +
        ...
        private async void btnDisconnect_Click(object sender, EventArgs e)
        {
            ...
            {
                logger.ErrorException(ex, "MQTT断开失败");
        ...
        private async void btnReconnect_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // 重置重连计数
                mqttService.ResetReconnectAttempts();
                ...
                logger.Log(LogLevel.Info, "MQTT重连成功");
            }
            catch (Exception ex)
            {
                logger.ErrorException(ex, "MQTT重连失败");
        ...
        private async void btnTestSend_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                await mqttService.PublishAsync(topic, json);

                logger.Log(LogLevel.Info, $"✅ 测试消息已发送到主题: {topic}");
                MessageBox.Show($"测试消息发送成功\n\n主题: {topic}\n\n内容:\n{json}", "成功", 
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
                logger.ErrorException(ex, "发送测试消息失败");
        ...
        protected override void OnFormClosing(FormClosingEventArgs e)
        {
            try
            {
                ...
                logger.Log(LogLevel.Info, "MQTT管理页面关闭");
            }
            catch (Exception ex)
            {
                logger.ErrorException(ex, "关闭MQTT管理页面失败");
            }
        ...
        private async Task ConnectTcpSocket()
        {
            try
            {
                ...
                logger.Log(LogLevel.Info, $"正在连接TCP服务器: {host}:{port}");
                ...
                logger.Log(LogLevel.Info, $"✅ TCP连接成功: {host}:{port}");
                MessageBox.Show($"TCP连接成功！\n\n服务器: {host}:{port}\n心跳: {(tcpEnableHeartbeat ? "启用" : "禁用")}", 
                    "成功", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
                logger.ErrorException(ex, "TCP连接失败");
        ...
        private void DisconnectTcpSocket()
        {
            try
            {
                ...
                logger.Log(LogLevel.Info, "TCP已断开");
            }
            catch (Exception ex)
            {
                logger.ErrorException(ex, "断开TCP失败");
        ...
        private async Task SendTcpData(string data)
        {
            try
            {
                ...
                logger.Log(LogLevel.Info, $"TCP已发送: {data}");
            }
            catch (Exception ex)
            {
                logger.ErrorException(ex, "TCP发送失败");
                MessageBox.Show($"发送失败：{ex.Message}", "错误", 
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void TcpSocket_ConnectionStatusChanged(object sender, bool isConnected)
        {
            ...
            logger.Log(LogLevel.Info, $"TCP状态变化: {(isConnected ? "已连接" : "已断开")}");
        }
        ...
        private void TcpSocket_TextReceived(object sender, string text)
        {
            ...
            logger.Log(LogLevel.Debug, $"TCP接收: {text}");
        }
        ...
        private void TcpSocket_ErrorOccurred(object sender, string error)
        {
            ...
            logger.Log(LogLevel.Error, $"TCP错误: {error}");
        }
