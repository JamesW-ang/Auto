    ...
    public class DatabaseHelper
    {
        ...
        private DatabaseHelper()
        {
            try
            {
                ...
                Gvar.Logger.Info($"[数据库] 数据库路径: {databasePath}");
                Gvar.Logger.Info($"[数据库] 文件存在: {File.Exists(databasePath)}");

                // 确保表结构存在
                EnsureTablesExist();
                
                Gvar.Logger.Info("[数据库] 数据库初始化成功");
            }
            catch (Exception ex)
            {
                Gvar.Logger.Error($"[数据库] 初始化失败: {ex.Message}");
                Gvar.Logger.Error($"[数据库] 堆栈: {ex.StackTrace}");
                throw;
            }
        }
        ...
        private void EnsureTablesExist()
        {
            try
            {
                ...
                {
                    Gvar.Logger.Info("[数据库] 正在创建数据库文件");
                ...
                using (var conn = GetConnection())
                {
                    ...
                    Gvar.Logger.Info("[数据库] 表结构验证完成");
                }
            }
            catch (Exception ex)
            {
                Gvar.Logger.Error($"[数据库] 确保表结构失败: {ex.Message}");
                throw;
            }
        }
        ...
        private void CreateTables(SQLiteConnection conn)
        {
            ...
            Gvar.Logger.Info("[数据库] 初始化表结构...");
            ExecuteNonQuery(conn, createUsersTable);
            Gvar.Logger.Info("[数据库] Users 表已创建");
            
            ExecuteNonQuery(conn, createLogsTable);
            Gvar.Logger.Info("[数据库] Logs 表已创建");
            
            ExecuteNonQuery(conn, createAlarmsTable);
            Gvar.Logger.Info("[数据库] Alarms 表已创建");
            
            ExecuteNonQuery(conn, createProductionTable);
            Gvar.Logger.Info("[数据库] ProductionData 表已创建");
            
            ExecuteNonQuery(conn, createConfigTable);
            Gvar.Logger.Info("[数据库] SystemConfig 表已创建");
        }
        ...
        private void InsertDefaultUsers(SQLiteConnection conn)
        {
            try
            {
                ...
                using (var cmd = new SQLiteCommand(insertUsers, conn))
                {
                    ...
                    Gvar.Logger.Info($"[数据库] 插入默认用户: {affected} 条");
                }
            }
            catch (Exception ex)
            {
                Gvar.Logger.Error($"[数据库] 插入默认用户失败: {ex.Message}");
            }
        }
        ...
        private void InsertDefaultConfig(SQLiteConnection conn)
        {
            try
            {
                ...
                using (var cmd = new SQLiteCommand(insertConfig, conn))
                {
                    cmd.Parameters.AddWithValue("@time", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
                    int affected = cmd.ExecuteNonQuery();
                    Gvar.Logger.Info($"[数据库] 插入默认配置: {affected} 条");
                }
            }
            catch (Exception ex)
            {
                Gvar.Logger.Error($"[数据库] 插入默认配置失败: {ex.Message}");
            }
        }
        ...
        public void CleanupOldLogs()
        {
            ...
            {
                Gvar.Logger.Error($"[数据库] 清理旧日志失败: {ex.Message}");
            }
        }
        ...
        public bool BackupDatabase(string backupPath = null)
        {
            ...
            {
                Gvar.Logger.Error($"[数据库] 备份数据库失败: {ex.Message}");
                return false;
            }
        }
